...
import { addPublicationapi } from './api'
...
function AddPublication() {
    ...
    // //    L'image est appropriée, mettre à jour l'état de la variable publication
    //                   setPublication({
    //                     ...publication,
    //                     image: e.target.files[0]
    //                   });
    ...
    //     // Vérifier si l'image est inappropriée avec Clarifai
    //     const reader = new FileReader();
    //     reader.readAsDataURL(e.target.files[0]);
    //     reader.onload = () => {
    //         app.models.predict(Clarifai.NSFW_MODEL, { base64: reader.result.split(',')[1] })
    //             .then(response => {
    //                 const concepts = response.outputs[0].data.concepts;
    //                 // Parcourir les concepts pour trouver le concept "racy"
    //                 for (let i = 0; i < concepts.length; i++) {
    //                     if (concepts[i].name === 'racy' && concepts[i].value > 0.8) {
    //                         alert('L\'image est inappropriée');
    //                         setIsBedImage(true)

    //                         return;
    //                     }
    //                 }
    //                 alert('L\'image est bonne');
    //                 setIsBedImage(false)
            ...
            await schema.validate(publication, { abortEarly: false })
                .then(async () => {
                    ...
                    if (!containsProfanity && !containsProfanityTitle) {
                        ...
                        const res = await addPublicationapi(userFromLocalStorage._id, formData)
                        notify()
                                ...
                                <div className="card bg-secondary shadow">
                                    <div className="card-header bg-white border-0">
                                        <div className="row align-items-center">
                                            <div className="col-8">
                                                <h3 className="mb-0">Ajouter publication</h3>
                                            </div>
                                    ...
                                    <div className="card-body">
                                        <form onSubmit={handleSubmit} enctype="multipart/form-data">
                                            ...
                                            <div className="col-12 text-right mr-5">
                                                ...
                                                <button  style={{backgroundColor: "#FF6B55",border : "none"}} type="submit" className="btn btn-sm    btn-primary">
